<div class="h-full flex flex-col bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700">
    ...
    <div class="flex items-center justify-between px-2 sm:px-3 py-2 border-b border-gray-200 dark:border-gray-700">
        ...
        <a href="{{ route('admin.warehouse.stock.index') }}" class="text-xs text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 hidden sm:block">
            Подробнее →
        </a>
    </div>
    ...
    <div class="flex-1 p-1 sm:p-2">
        ...
        <div class="grid grid-cols-3 gap-1 sm:gap-2 mb-1 sm:mb-2">
            <div class="bg-blue-50 dark:bg-blue-900/20 p-1.5 sm:p-2 rounded">
                ...
                <div class="text-xs sm:text-sm font-bold text-blue-700 dark:text-blue-300">{{ $totalProducts }}</div>
                @if($lowStockProducts > 0)
                <div class="text-xs text-red-600 dark:text-red-400 hidden sm:block">{{ $lowStockProducts }} на исходе</div>
                @endif
            </div>
            ...
            <div class="bg-purple-50 dark:bg-purple-900/20 p-1.5 sm:p-2 rounded">
                ...
                <div class="text-xs sm:text-sm font-bold text-purple-700 dark:text-purple-300">{{ $totalElements }}</div>
                @if($lowStockElements > 0)
                <div class="text-xs text-red-600 dark:text-red-400 hidden sm:block">{{ $lowStockElements }} на исходе</div>
                @endif
            </div>
        ...
        <div class="sm:hidden">
            <div class="text-center p-2 bg-red-50 dark:bg-red-900/20 rounded">
                <div class="text-xs text-red-600 dark:text-red-400">⚠️ Критических остатков: {{ count($criticalProducts) }}</div>
            </div>
        </div>
        @endif
    </div>
